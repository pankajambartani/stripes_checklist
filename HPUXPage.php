 ...
 <body>
 	<div id="topbar"><h2>HP-UX Products</h2><a id="back" href="index.php">Back</a> <a id="logout" href="logout.php">Log Out</a></div>
 	<div id="w">
 		<div id="content">
 			<h1 style="font-size: 150%;"><?php echo $_GET['product_name'];?></h1>
 			<form method="GET" action="HPUXPage.php">
 			...
 			{		
				$product_name = $_GET['product_name'];
 				...
 				$sql_FinalSubmitCheck = "SELECT bug_id FROM hpuxanswer where bug_id='$bugId'";  //Answers database committed data
				...
				$sql_idcheck = "SELECT bug_id FROM hpuxtemp where bug_id=".$bugId;     //Answers database for storing submitted data
				...
				{
					$sql_bugs= "SELECT bug_id, short_desc, assigned_to, Remark FROM hpuxtemp where bug_id=".$bugId;
					$bug_result = mysqli_query($conn, $sql_bugs) or die(mysqli_error($conn));
					$rowInitial = mysqli_fetch_assoc($bug_result);

					$sql_quetionId = "SELECT question_id FROM hpuxquestion WHERE product_name='$product_name'";
					$questionId_result = mysqli_query($conn, $sql_quetionId) or die(mysqli_error($conn));
						...
						<table>
							...
							<tbody>
								...
								while($row = $Result_sql_questions->fetch_assoc()) {
									// echo $arr_questionId[$i++]['question_id'];
									echo '<tr>';
					...
					{	 
						...
						$sql_question = "SELECT * FROM hpuxquestion where product_name='".$product_name."'";
						$result_question = mysqli_query($conn, $sql_question) or die(mysqli_error($conn));
						?>
						<form name="myFormDraftSubmit" method="post" action="HPUXSubmitData.php">
							<table>
								<thead>
									<tr>
										<th>Questions</th>

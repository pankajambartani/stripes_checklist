...
<body>
	<div id="topbar"><h2>HPUX Products</h2><a id="back" href="index.php">Home</a></div>
	<div id="w">
		<div id="content">
			...
			$sql_bugs= "SELECT bug_id, short_desc, assigned_to, Remark FROM hpuxanswer where bug_id=".$bugId;
			$bug_result = mysqli_query($conn, $sql_bugs) or die(mysqli_error($conn));
			$rowInitial = mysqli_fetch_assoc($bug_result);

			$sql_questionId = "SELECT question_id FROM hpuxquestion WHERE product_name='$product_name'";
			...
			$sql = substr($sql ,0,-1);
			$sql_answer .= $sql ." FROM hpuxanswer WHERE bug_id=".$bugId;

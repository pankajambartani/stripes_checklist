 ...
 <body>
 	...
 	<div id="w">
 		<div id="content">
 			...
 			<form method="GET" action="NonStopPage.php">
 			...
 			{		
				...
				$sql_FinalSubmitCheck = "SELECT bug_id FROM nonstopanswer where bug_id=".$bugId;     //Answers database for storing submitted data
				...
				$sql_idcheck = "SELECT bug_id FROM nonstoptemp where bug_id=".$bugId;     //Answers database for storing submitted data
				...
				{
					$sql_bugs= "SELECT bug_id, short_desc, assigned_to, Remark FROM nonstoptemp where bug_id=".$bugId;
					$bug_result = mysqli_query($conn, $sql_bugs) or die(mysqli_error($conn));
					$rowInitial = mysqli_fetch_assoc($bug_result);

					$sql_questionId = "SELECT question_id FROM nonstopquestion WHERE product_name='$product_name'";
					...
					$sql = substr($sql ,0,-1);
					$sql_answer .= $sql ." FROM nonstoptemp WHERE bug_id=".$bugId;
					$Result_sql_answer = mysqli_query($conn, $sql_answer) or die(mysqli_error($conn)); // Submitted answers
					
					$sql_quetions = "SELECT question FROM nonstopquestion WHERE product_name='$product_name'";
					$Result_sql_questions = mysqli_query($conn, $sql_quetions) or die(mysqli_error($conn)); // question from question table
					?>
					<form name="myFormSubmit" method="post" action="NonStopUpdateData.php">
						<table>
							<thead>
								<tr>
									<th>Questions</th>
					...
					{	 
						...
						$sql_question = "SELECT * FROM nonstopquestion where product_name='".$product_name."'";
						$result_question = mysqli_query($conn, $sql_question) or die(mysqli_error($conn));
						?>
						<form name="myFormDraftSubmit" method="post" action="NonStopSubmitData.php">
							<table>
								<thead>
									<tr>
										<th>Questions</th>
